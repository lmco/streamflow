...
@Singleton
public class MongoSerializationDao extends MongoDao<Serialization, String>
        implements SerializationDao {

    @Inject
    public MongoSerializationDao(Datastore datastore) {
        super(datastore, Serialization.class);
    }

    @Override
    public List<Serialization> findAll() {
        return query().order("typeClass").asList();
    }

    @Override
    public List<Serialization> findAllWithFramework(String framework) {
        return query().field("framework").equal(framework).order("typeClass").asList();
    }

    @Override
    public Serialization findByTypeClass(String typeClass) {
        return query().field("typeClass").equal(typeClass).get();
    }
}
